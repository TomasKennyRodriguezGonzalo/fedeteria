...
pub fn edit_personal_info_molecule() -> Html {
    ...
    let name_state = use_state(|| "default".to_string());   
    let email_state = use_state(|| None);    
    let birth_date_state = use_state(|| default_local_date);    
    ...
    use_effect_once(move || {
        let name_state = name_state_c;
        let email_state = email_state_c;
        let birth_date_state = birth_date_state_c;
        let query = QueryGetUserInfo { dni: dni.unwrap() };
        request_post("/api/get_user_info", query, move |response: ResponseGetUserInfo| {
            name_state.set(response.nombre_y_ap);
            email_state.set(Some(response.email));
            birth_date_state.set(response.nacimiento);
        });
    ...
    let email_changed = Callback::from(move |new_email: Option<String>| {
        email_state_c.set(new_email);
    });
